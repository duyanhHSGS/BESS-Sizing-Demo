...
def constrain_charge_to_cheap_window(
    ...
    *,
    ...
    """Block charging outside cheap slots plus explicitly allowed extra slots."""
    action_value = float(action)
...
def charge_window_steps(
    ...
    *,
    ...
    if isinstance(steps_per_day, bool) or int(steps_per_day) != steps_per_day:
        raise ValueError("charge window steps_per_day must be a positive integer")
    ...
    # TODO(IQ-77): keep this half-open: the 17:15 sample may charge through
    # 17:30, while a new action at exactly 17:30 is blocked.
...
def step_brain_control(
    ...
    *,
    ...
    if additional_charge_steps and not charge_only_during_cheap_tariff:
        raise ValueError("additional charge permission requires the charge-window constraint")
