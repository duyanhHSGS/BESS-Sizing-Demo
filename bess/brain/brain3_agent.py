...
class Brain3ReplayBuffer:
    ...
    def state_dict(self) -> dict[str, Any]:
        """Serialize only initialized replay rows while preserving ring position and RNG."""
        return {
            ...
            "rewards_vnd": self._rewards_vnd[: self._size].copy(),
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        if int(state.get("capacity", -1)) != self.capacity or int(
            state.get("observation_dim", -1)
        ) != self.observation_dim:
            raise ValueError("Brain3 replay shape does not match resume configuration")
        size = int(state.get("size", -1))
        write_index = int(state.get("write_index", -1))
        if not 0 <= size <= self.capacity or not 0 <= write_index < self.capacity:
            raise ValueError("Brain3 replay resume counters are invalid")
        for target, key in (
            (self._observations, "observations"),
            (self._actions, "actions"),
            (self._rewards_vnd, "rewards_vnd"),
            (self._next_observations, "next_observations"),
            (self._dones, "dones"),
        ):
            values = np.asarray(state[key])
            if len(values) != size:
                raise ValueError(f"Brain3 replay {key} row count is incompatible")
            target[:size] = values
        self._size = size
...
class Brain3Agent:
    ...
    def __init__(
        ...
        *,
        ...
        self.online_network = Brain3QNetwork(hidden_dim=self.hidden_dim).to(self.device)
        self.target_network = Brain3QNetwork(hidden_dim=self.hidden_dim).to(self.device)
    ...
    def load_checkpoint(self, state: dict[str, Any]) -> None:
        if state.get("algorithm") != "dqn" or int(state.get("observation_dim", -1)) != OBSERVATION_DIM:
            raise ValueError("Brain3 resume checkpoint has an incompatible algorithm schema")
        if tuple(state.get("action_values", ())) != BRAIN3_ACTIONS:
            raise ValueError("Brain3 resume checkpoint has an incompatible action schema")
